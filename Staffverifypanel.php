...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['send_verify_id'])) {
    ...
    $overdueCheck = $conn->query("SELECT (DATE_ADD(Pickup_Date, INTERVAL 11 HOUR) < NOW()) AS overdue FROM booking WHERE Booking_ID = $bid LIMIT 1");
    if ($overdueCheck && $overdueCheck->num_rows > 0 && (int)$overdueCheck->fetch_assoc()['overdue'] === 1) {
        header("Location: staffVerifyPanel.php?msg=has_penalty");
        exit();
    }
...
if ($search !== '') {
    ...
    $filterWhere .= " AND b.Booking_ID LIKE '%$searchEsc%'";
}
...
$bookings = $conn->query("
    SELECT b.Booking_ID, b.Pickup_Date, b.DropOff_Date, b.Booking_Status, b.Dropoff_Status,
           ...
           (DATE_ADD(b.Pickup_Date, INTERVAL 11 HOUR) < NOW()) AS IsOverduePenalty
    FROM booking b
...
<head>
    ...
    <style>
        ...
        .search-clear { color: #8b82b5; font-size: 0.82rem; line-height: 2.4; text-decoration: none; }
...
<button class="back" onclick="history.back()">&#60; Back</button>
    ...
    <div class="rightcontainer">
        ...
        <?php if (isset($_GET['msg'])):
            ...
            if (in_array($_GET['msg'], ['outside_hours', 'no_photo', 'has_penalty'])) $msgClass = 'msg-error';
        ?>
        <div class="msg-banner <?php echo $msgClass; ?>">
            <?php
                $msgs = [
                    ...
                    'has_penalty' => 'This booking has an overdue penalty. It must be paid by the student or waived by staff before sending verification.',
                ];
                echo $msgs[$_GET['msg']] ?? '';
            ?>
        </div>
        ...
        <form method="GET" class="search-row">
            <input type="text" class="search-input" name="q" placeholder="Search by Booking ID..." value="<?php echo htmlspecialchars($search); ?>">
            <input type="hidden" name="filter" value="<?php echo htmlspecialchars($filter); ?>">
            <button type="submit" class="search-btn">Search</button>
            <?php if ($search !== ''): ?>
                <a href="staffVerifyPanel.php?filter=<?php echo urlencode($filter); ?>" class="search-clear">Clear</a>
            <?php endif; ?>
        </form>

        <!-- Filters -->
        <div class="filter-row">
            <a href="staffVerifyPanel.php?filter=all&q=<?php echo urlencode($search); ?>"       class="filter-chip <?php echo $filter==='all'       ? 'active' : ''; ?>">All</a>
            <a href="staffVerifyPanel.php?filter=notsent&q=<?php echo urlencode($search); ?>"   class="filter-chip <?php echo $filter==='notsent'   ? 'active' : ''; ?>">Not Sent</a>
            <a href="staffVerifyPanel.php?filter=pending&q=<?php echo urlencode($search); ?>"   class="filter-chip <?php echo $filter==='pending'   ? 'active' : ''; ?>">Awaiting Student</a>
            <a href="staffVerifyPanel.php?filter=confirmed&q=<?php echo urlencode($search); ?>" class="filter-chip <?php echo $filter==='confirmed' ? 'active' : ''; ?>">Student Confirmed</a>
            <a href="staffVerifyPanel.php?filter=released&q=<?php echo urlencode($search); ?>"  class="filter-chip <?php echo $filter==='released'  ? 'active' : ''; ?>">Awaiting Collection Confirm</a>
        </div>

        <div class="section-label">
            Bookings
            <?php if ($search !== ''): ?>
                — showing results for Booking ID "<?php echo htmlspecialchars($search); ?>"
            <?php endif; ?>
        </div>
        ...
        <div class="verify-card">
            <div class="verify-top">
                ...
                <?php endif; ?>
                <?php if ($row['IsOverduePenalty']): ?>
                    <span class="overdue-flag"> Overdue Penalty Pending</span>
                <?php endif; ?>
            </div>
            ...
            <div class="action-row">
                ...
                <?php elseif ($row['Dropoff_Status'] === 'Confirmed' && $withinWorkingHours && !$row['IsOverduePenalty']): ?>
                    <form method="POST" style="display:inline">
                ...
                <?php else: ?>
                    <button class="btn-send" disabled
                        title="<?php
                            if ($row['IsOverduePenalty']) echo 'Overdue penalty must be paid or waived first';
                            elseif ($row['Dropoff_Status'] !== 'Confirmed') echo 'Drop-off photo must be uploaded and confirmed by the student first';
                            else echo 'Only available 8:00 AM - 11:00 AM';
                        ?>">
                        Send Verification →
                    </button>
                    <span class="verify-blocked-note">
                        <?php
                            if ($row['IsOverduePenalty']) {
                                echo 'Overdue penalty must be paid or waived first';
                            } elseif ($row['Dropoff_Status'] !== 'Confirmed') {
